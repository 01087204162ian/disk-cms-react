...
## 📋 실무자 논의 후 결정 사항 (2026-02-02)

| 항목 | 현재 상태 | 검토 필요 |
|------|----------|----------|
| **갱신리스트 만기 기간** | `DATEDIFF BETWEEN -45 AND 45` (과거 45일 + 미래 45일) | "오늘부터 45일 안에 만기" = `0~45`(미래만)로 변경 여부 |
| **건수 차이** | 101건 조회 vs 예상 38건 | 38건(승인−해지)과 갱신리스트 기준 정의 일치 여부 확인 |
| **일별실적 갱신 대상** | 승인 47 − 해지 9 = 38 | 갱신리스트와 동일 집계 기준인지 검증 필요 |
