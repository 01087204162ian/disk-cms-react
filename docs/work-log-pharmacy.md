...
### 2026-02-24 (약국배상책임보험) - 승인→메일보냄 정산/예치 처리 로직 정리

#### 작업 내용
- **기능**: Applications에서 상태를 승인(13)에서 메일보냄(10)으로 되돌릴 때 정산/예치 처리 방식을 명확히 정리
- **파일**: `imet/api/pharmacy/pharmacy-status-update.php`
- **주요 구현 사항**:
  - ✅ 메일보냄(10) 처리 시 `pharmacy_settlementList` 기존 승인 정산 레코드(`sort = 13`)를 **INSERT가 아닌 UPDATE**로 `sort = 7`(취소/보류)로 변경
  - ✅ 같은 분기에서 실제 산출 보험료를 기준으로 `pharmacy_deposit` 잔고(`sort = 98`)에 보험료를 다시 **+** 하여 예치금 환급 처리
...
- 추후 실적/정산 리포트에서 승인 건과 취소/보류(메일보냄) 건을 명확히 구분할 수 있는 기반 마련
