...
### 2026-02-24 (약국배상책임보험) - 승인→메일보냄/보류 정산·예치 처리 로직 정리

#### 작업 내용
- **기능**: Applications에서 상태를 승인(13)에서 메일보냄(10) 또는 보류(7)로 되돌릴 때 정산/예치 처리 방식을 명확히 정리
- **파일**: `imet/api/pharmacy/pharmacy-status-update.php`
- **주요 구현 사항**:
  - ✅ **승인(13) → 메일보냄(10)**:  
    - `pharmacy_settlementList` 기존 승인 정산 레코드(`sort = 13`)를 **INSERT가 아닌 UPDATE**로 `sort = 7`(취소/보류)로 변경  
    - 실제 산출 보험료(전문인+화재)를 기준으로 `pharmacy_deposit` 잔고(`sort = 98`)에 보험료를 다시 **+** 하여 예치금 환급 처리
  - ✅ **승인(13)/계약완료(6)/증권발급(14) → 보류(7)**:  
    - `recalculatePremiumForRefund()`로 전문인/화재/총 보험료를 다시 계산한 뒤, 그 금액으로 `pharmacy_settlementList`에 `sort = 7` 정산 레코드 INSERT  
    - 같은 금액만큼 `pharmacy_deposit` 잔고(`sort = 98`)에 **+** 하여 예치금 환급 처리 (보류=보험료 전액 환급 개념)
  - ✅ 문서상 v2 API(`/hi/api/pharmacy-status-update_v2.php`)와 내부용 PHP(`pharmacy-status-update.php`)의 역할 차이를 재정리 (v2는 HMAC 외부용, 내부 정산·예치는 PHP 본체에서 처리)

#### 결과
- 승인(13) → 메일보냄(10) 변경 시, 정산 이력은 `13` → `7` 한 건으로 정리되고 예치금은 승인 이전 수준으로 복원됨
- 승인/계약완료/증권발급 → 보류(7) 변경 시, 재계산된 보험료 기준으로 정산·예치가 일관되게 환급 처리됨
- 추후 실적/정산 리포트에서 승인 건과 취소/보류(메일보냄 포함) 건을 명확히 구분할 수 있는 기반 마련
