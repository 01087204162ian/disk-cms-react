...
### 2026-01-26 (KJ 대리운전) - 배서리스트 페이지 UI 개선 및 조건부 색상 표시 기능 추가

#### 작업 내용
- **기능**: 배서리스트 페이지의 컬럼 폭 조정 및 성명 컬럼에 조건부 색상 표시 기능 추가
- **파일**: 
  - `disk-cms-react/src/pages/insurance/EndorseList.tsx` (컬럼 폭 조정, 조건부 색상 표시)
- **주요 구현 사항**:
  - ✅ 컬럼 폭 조정
    - 주민번호 컬럼: `w-44` (176px) → `w-40` (160px) - 약 10% 감소
    - 요율 컬럼: `w-20` (80px) → `w-24` (96px) → `w-32` (128px) - 총 48px 증가
    - 증권번호 컬럼: `w-36` (144px) → `w-32` (128px) - 16px 감소
  - ✅ 성명 컬럼 조건부 색상 표시 기능 추가
    - **28세 미만 (거절자)**: 빨간색 (`text-red-600`) + `font-semibold`
    - **중복 (주민번호 중복)**: 파란색 (`text-blue-600`) + `font-semibold`
      - 동일한 주민번호의 기사가 해지 또는 청약한 증권 이외에 다른 증권 또는 다른 대리운전 회사에 가입되어 있는 경우
    - **동명이인**: 보라색 (`text-purple-600`) + `font-semibold`
      - 배서리스트 내에서 같은 이름이 2개 이상인 경우
  - ✅ 우선순위 적용: 중복(주민번호) > 동명이인 > 28세 미만
  - ✅ 동명이인 체크 로직 구현
    - `nameCountMap`을 사용하여 현재 배서리스트에서 이름별 카운트 계산
    - 같은 이름이 2개 이상인 경우 동명이인으로 판단

#### 개선 효과
- 컬럼 폭 조정으로 요율 컬럼의 가독성 향상
- 조건부 색상 표시로 거절자, 중복, 동명이인을 시각적으로 구분 가능
- 실무자가 중요한 정보를 빠르게 파악할 수 있도록 개선

---

### 2026-01-24 (KJ 대리운전) - 운영 매뉴얼(문서) 페이지 구축 및 업무흐름 중심 문서화

#### 작업 내용
- **기능**: 약국배상 `pharmacy/documentation` 스타일을 참고하여 KJ 대리운전도 **문서(매뉴얼)** 메뉴/페이지를 추가하고, 실무자 관점의 **업무흐름(기사추가→배서→해지/취소→정산/통계→문자검증)** 중심으로 매뉴얼을 확장
- **파일**:
  - `disk-cms-react/src/pages/insurance/Documentation.tsx` (KJ 대리운전 운영 매뉴얼 페이지 추가/확장 v0.6)
  - `disk-cms-react/src/App.tsx` (라우트 추가: `/insurance/kj-driver-documentation`)
  - `disk-cms-react/public/config/menu-config.json` (KJ대리운전 하위 메뉴에 **문서(매뉴얼)** 추가)
- **주요 구현 사항**:
  - ✅ 메뉴 추가: **보험상품 > 대리운전 > KJ대리운전 > 문서(매뉴얼)**
  - ✅ 문서 페이지 뼈대 → 섹션 단위로 확장 (너무 길어지지 않게 분할 방식)
  - ✅ 업무흐름(실무) 섹션 신설
    - 대리기사 추가(청약) → 동의(SMS) → 배서 처리
    - 해지/해지취소(배서 포함)
    - 정산/통계(증권/담당자 기준) 및 SMSData 기반 검증 포인트
  - ✅ API/DB 요약 섹션 추가
    - 화면 → API 매핑(요약) + 자주 쓰는 엔드포인트 요약
    - 핵심 테이블/관계/자주 보는 필드 요약 + `pci0327/kj/docs/*` 참조 안내
  - ✅ FAQ/트러블슈팅 섹션 추가(최근 이슈 중심)
    - 검색 클릭 시 circular JSON 에러 원인/대응(이벤트 객체 전달 방지)
    - 빌드(TS6133) 미사용 변수 에러 대응(제거/사용 처리)

#### 참고 사항
- 고객사(기존) 소스/문서 기준: `pci0327/kj/docs/*`, `pci0327/kj/*.html`, `pci0327/kj/js/*`
- 운영 URL(React): `/insurance/kj-driver-documentation`
